...
test('public snapshots have complete unique flood features, energy provenance and positive stock',()=>{
 ...
 const flood=load('flood.geojson');assert.equal(flood.features.length,flood.metadata.count);assert.equal(new Set(flood.features.map(f=>f.properties.OBJECTID)).size,flood.features.length);assert.ok(flood.features.every(f=>['06081C','06085C'].includes(f.properties.DFIRM_ID)));
 ...
 const stock=load('housing-stock.json');assert.equal(Object.keys(stock.cities).length,33);assert.equal(stock.cities.sunnyvale.units,61272);assert.ok(Object.values(stock.cities).every(c=>c.units>0&&c.year===2020));
