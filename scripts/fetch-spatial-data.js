...
if(task==='energy'){
 ...
 const meta=await json(base+'?f=json');const url=new URL(base+'/query');url.search=new URLSearchParams({f:'geojson',where:"County IN ('Santa Clara','San Mateo','Alameda','Contra Costa','San Francisco','Marin','Napa','Solano','Sonoma')",outFields:'CECPlantID,PlantName,Retired_Plant,County,Capacity_Latest,PriEnergySource',outSR:'4326',resultRecordCount:'3000'});
 ...
 const where="(SFHA_TF='T' OR ZONE_SUBTY LIKE '%0.2%') AND (DFIRM_ID LIKE '06081%' OR DFIRM_ID LIKE '06085%' OR DFIRM_ID LIKE '06001%' OR DFIRM_ID LIKE '06013%' OR DFIRM_ID LIKE '06075%' OR DFIRM_ID LIKE '06041%' OR DFIRM_ID LIKE '06055%' OR DFIRM_ID LIKE '06095%' OR DFIRM_ID LIKE '06097%')";
 ...
 save('flood.geojson',{type:'FeatureCollection',features,metadata:{source:base,agency:'FEMA National Flood Hazard Layer',retrieved,count:count.count,requestedCounties:['San Mateo','Santa Clara','Alameda','Contra Costa','San Francisco','Marin','Napa','Solano','Sonoma'],counties:[...new Set(features.map(f=>({'06081C':'San Mateo','06085C':'Santa Clara','06001C':'Alameda','06013C':'Contra Costa','06075C':'San Francisco','06041C':'Marin','06055C':'Napa','06095C':'Solano','06097C':'Sonoma'}[f.properties.DFIRM_ID])))],coverageNote:'The query includes nine counties. Only counties returning mapped hazard polygons are listed as available. Missing polygons do not establish low risk.',description:'Special flood hazard and 0.2% annual-chance zones. Geometry generalized to approximately 3 m for screening; not an insurance or parcel determination. No match is not no risk.'}});
 ...
 for(let i=0;i<cities.length;i+=4){await Promise.all(cities.slice(i,i+4).map(async city=>{
  ...
  const slug=catalog.cities.find(c=>c.key===city)?.censusSlug||label.toLowerCase().replaceAll(' ','_');
