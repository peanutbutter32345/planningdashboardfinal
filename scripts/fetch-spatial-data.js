...
if(task==='energy'){
 ...
 const meta=await json(base+'?f=json');const url=new URL(base+'/query');url.search=new URLSearchParams({f:'geojson',where:"County IN ('Santa Clara','San Mateo')",outFields:'CECPlantID,PlantName,Retired_Plant,County,Capacity_Latest,PriEnergySource',outSR:'4326',resultRecordCount:'3000'});
 ...
 const where="(SFHA_TF='T' OR ZONE_SUBTY LIKE '%0.2%') AND (DFIRM_ID LIKE '06081%' OR DFIRM_ID LIKE '06085%')";
 ...
 save('flood.geojson',{type:'FeatureCollection',features,metadata:{source:base,agency:'FEMA National Flood Hazard Layer',retrieved,count:count.count,counties:['San Mateo','Santa Clara'],description:'Special flood hazard and 0.2% annual-chance zones. Geometry generalized to approximately 3 m for screening; not an insurance or parcel determination. No match is not no risk.'}});
}else if(task==='housing'){
 const cities=[...new Set(dashboardProjects().map(p=>p.city))];const data={};
 for(let i=0;i<cities.length;i+=4){await Promise.all(cities.slice(i,i+4).map(async city=>{
  ...
  const label=name||names[city]||city;
